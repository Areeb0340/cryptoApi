...
class MarketDataEngine:
    ...
    def process_depth(self, data):
        ...
        cache.orderbooks[symbol] = {
            "bids": [{"price": float(p), "qty": float(q)} for p, q in data["b"]],
            "asks": [{"price": float(p), "qty": float(q)} for p, q in data["a"]],
            "time": data["T"],
        }
    ...
    def process_kline(self, data):
        print("MARKET DATA ENGINE CALLED")

        async def __call__(self, data):
            print("KLINE EVENT:", data["s"], data["k"]["i"], data["k"]["x"])
